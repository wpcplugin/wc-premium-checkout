...
class Card_Selector extends \WP_Customize_Control {
	...
	?>		
		...
		<# _.each( data.cards, function( card, id ) { #>
			...
			<div class="wpc-control-card <# if ( card.isActivecard ) { #>active <# } #>" <# if ( card.action.value ) { #> data-value="{{ card.action.value }}" <# } #> <# if ( card.action.active_text ) { #> data-active-text="{{ card.action['active_text'] }}" <# } #> <# if ( card.action.text ) { #> data-text="{{ card.action['text'] }}" <# } #> <# if ( card.action.link ) { #> data-url="{{ card.action['link'] }}" <# } #> data-id="{{ card.action['id'] }}">
